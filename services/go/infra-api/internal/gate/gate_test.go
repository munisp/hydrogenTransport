...
// infra-api gates the incident routes behind the leak-detection module.
const (
	moduleUnderTest = "leak-detection"
	routeUnderTest  = "/v1/incidents"
)
...
func toggleServer(t *testing.T, enabled bool) *httptest.Server {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			"module": module, "enabled": enabled, "domain": "infra",
		})
	}))
	t.Cleanup(srv.Close)
	return srv
