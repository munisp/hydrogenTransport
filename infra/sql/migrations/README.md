...
| `0010_wave7.sql` | Wave-7 DECISION builds (plan-wave7.md): `commerce.billing_accounts` + `billing_charges` + `invoices` + `rider_entitlements.payer_account` (corporate/employer group settlement, A2-06), `infra.charter_bookings` + `charter_vehicles` (multi-vehicle block reservations backed by `charter:<ref>` dispatch jobs, A2-09). |
| `0011_wave8_onboarding.sql` | Wave-8 onboarding hardening (plan-wave8.md): `platform.onboarding_requests` created IF NOT EXISTS (absorbs admin-api's runtime EnsureSchema), status CHECK re-added as a named constraint incl. `expired` (existing auto-named CHECK found + dropped via DO block), partial unique index `onboarding_requests_pending_uq (persona, email) WHERE status='pending'` (intake dedup backstop), `(email, created_at DESC)` index (velocity cap), status/persona list indexes. |
| `0012_wave9_onboarding.sql` | Wave-9 W9-2 (plan-wave9.md): email identity is case-insensitive — backfills `email = lower(email)` and rebuilds the 0011 pending-dedup unique index + velocity index as expression indexes on `lower(email)` so case variants cannot bypass dedup or the per-email cap. |
